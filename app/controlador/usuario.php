...
class Controlador_Usuario{
	...
	public function registrar_usuario(){
		...
		if ($permiso) {
			...
			if (!$resultados) {
				...
				echo'<script>alert("error, no se pudo insertar un nuevo usuario...")</script>';
	...
	public function cambiar_estado(){
		...
		echo'<script>alert("correcto,estado actual del usuario ha sido cambiado...")</script>';
	...
	public function editar_mis_datos(){
			...
			if ($actualizar) {
				echo '<script>alert("correcto, datos actualizados");</script>';
				echo '<script>window.location.href = "?controller=front&action=mi_perfil";</script>';
			}else{
				echo '<script>alert("error, datos no actualizados");</script>';
				echo '<script>window.location.href = "?controller=front&action=mi_perfil";</script>';
			}	
	...
	public function cambiar_mi_departamento(){
			...
			if ($actualizar) {
				echo'<script>alert("correcto, se actualizo el departamento actual...")</script>';
			echo '<script>window.location.href = "?controller=front&action=mi_perfil";</script>';
			}			
	}
	...
	public function cambio_de_categoria(){
			...
			if (!$resultado) {
				...
				$this->obj_usuario_trabajo->set_fk_trabajo($resultado->id_trabajo);
				$resultado_autores = $this->obj_usuario_trabajo->consultar_vinculo();
				$cantidad_autores = count($resultado_autores);
				
				if ($cantidad_autores > 0) {
					echo'<script>alert("Usuario ascendido con exito...")</script>';
			echo '<script>window.location.href = "?controller=front&action=detalles_usuario&id_usuario='.$id_usuario.'";</script>';
				}elseif($cantidad_autores == 0){
					...
					echo'<script>alert("El usuario ha sido ascendido con exito... como era el ultimo autor que faltaba por ascender el trabajo se cerrara automaticamente")</script>';
					echo '<script>window.location.href = "?controller=front&action=detalles_usuario&id_usuario='.$id_usuario.'";</script>';
				}
	...
	public function login(){
		...
		if (!$datos_usuario) {
			...
			if ($permiso == 0) {
				...
				echo '<script>alert("no tienes permisos para iniciar sesion");</script>';
				echo '<script>window.location.href = "?controller=front&action=home";</script>';
			}else{
	...
	public function correo($asunto = '',$cuerpo = '',$pie = '',$para = ''){
		...
		//Nuestra cuenta
		$this->obj_mail->Username ='sistemaautanauptaeb@gmail.com';
		$this->obj_mail->Password = 'autanauptaeb';
	...
	public function reestablecer_clave(){
		...
		$email = $_POST['correo'];
		$this->obj_usuario->set_correo($email);
		$result = Controlador_Usuario::validar_correo();
	...
	public function reestablecer_clave_a_terceros(){
			...
			$resultado_envio = Controlador_Usuario::correo($asunto,$cuerpo_mensaje,$pie_mensaje,$para);
				...
				if($resultado_envio){
					...
					$this->obj_usuario->set_clave($nueva_clave_encriptada);
					$cambiar = $this->obj_usuario->cambiar_clave();
					echo'<script>alert("correcto, le hemos enviado al usuario un correo con su nueva clave...")</script>';
					echo '<script>window.location.href = "?controller=front&action=detalles_usuario&id_usuario='.$id_usuario.'";</script>';
				}else{
					echo'<script>alert("El servidor experimenta errores a la hora de enviar el correo por favor intente mas tarde o revise su conexion a Internet...")</script>';
					echo '<script>window.location.href = "?controller=front&action=detalles_usuario&id_usuario='.$id_usuario.'";</script>';
				}
	}
	...
	public function cambio_de_clave(){
		...
		if ($resultado_clave) {
			...
			echo'<script>alert("la clave actual ingresada es incorrecta...")</script>';
			echo '<script>window.location.href = "?controller=front&action=mi_perfil";</script>';
		}
	}
